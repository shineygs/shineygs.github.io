...
startActivity(intent);
```
启动一个应用时：都会调用startActivity(Intent), 会通过Binder IPC机制, 最终调用到ActivityManagerService的startActivity函数
...
2. AMS在完成任务栈和生命周期管理后，回调App方法，这时AMS是client，App进程是Server，Server Binder对象就是ApplicationThread，

mToken是Activity中一个成员变量，可以唯一标示Activity。
...
```
这里把App中Binder Server端的ApplicationThread的句柄传给AMS，用于生命周期的回调等。
...
在AMS会新创建一个栈Task，并通过ApplicationThread的句柄通知Activity进入paused状态
...
> 通过Binder驱动程序调用ApplicationThread的scheduleLaunchActivity。然后利用mH.sendMessage最终会执行到ActivityThread中handleLaunchActivity函数，在这个函数主要是创建了Activity，并调用了attach和onCreate方法。
